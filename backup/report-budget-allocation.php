...
{
    $query = "WITH account_hierarchy AS (
                SELECT 
                    a1.account,
                    a1.account AS account1,
                    a1.alias_default AS level1,
                    a1.parent,
                    CAST(NULL AS NVARCHAR(255)) AS account2,
                    CAST(NULL AS NVARCHAR(255)) AS level2,
                    CAST(NULL AS NVARCHAR(255)) AS account3,
                    CAST(NULL AS NVARCHAR(255)) AS level3,
                    CAST(NULL AS NVARCHAR(255)) AS account4,
                    CAST(NULL AS NVARCHAR(255)) AS level4,
                    CAST(NULL AS NVARCHAR(255)) AS account5,
                    CAST(NULL AS NVARCHAR(255)) AS level5,
                    1 AS depth
                FROM account a1
                ...
                SELECT 
                    ...
                    ah.level1,
                    a2.parent,
                    CASE WHEN ah.depth = 1 THEN a2.account ELSE ah.account2 END,
                    CASE WHEN ah.depth = 1 THEN a2.alias_default ELSE ah.level2 END,
                    CASE WHEN ah.depth = 2 THEN a2.account ELSE ah.account3 END,
                    CASE WHEN ah.depth = 2 THEN a2.alias_default ELSE ah.level3 END,
                    CASE WHEN ah.depth = 3 THEN a2.account ELSE ah.account4 END,
                    CASE WHEN ah.depth = 3 THEN a2.alias_default ELSE ah.level4 END,
                    CASE WHEN ah.depth = 4 THEN a2.account ELSE ah.account5 END,
                    CASE WHEN ah.depth = 4 THEN a2.alias_default ELSE ah.level5 END,
                    ah.depth + 1
                FROM account_hierarchy ah
                JOIN account a2 
                    ON ah.parent = a2.account COLLATE Thai_CI_AS
                WHERE ah.parent IS NOT NULL AND ah.depth < 5
            ),
            ...
            SELECT 
                ...
                bap.Fund,
                bap.KKU_Item_Name,
                ISNULL(bap.Allocated_Total_Amount_Quantity, 0) AS Allocated_Total_Amount_Quantity,
                ISNULL(bpd.Release_Amount, 0) AS Release_Amount,

                -- Pre_Release_Amount ตาม Scenario
                CASE 
                    WHEN :selectedScenario = 'Scenario1' THEN 0
                    WHEN :selectedScenario = 'Scenario2' THEN 
                        CASE WHEN bpd.Scenario = 'ANL-RELEASE-1' THEN ISNULL(bpd.Release_Amount, 0) ELSE 0 END
                    WHEN :selectedScenario = 'Scenario3' THEN 
                        CASE 
                            WHEN bpd.Scenario IN ('ANL-RELEASE-1','ANL-RELEASE-2','ANL-RELEASE-3') THEN (
                                SELECT ISNULL(SUM(Release_Amount), 0)
                                FROM budget_planning_disbursement_budget_plan_anl_release bpd_sub
                ...
                -- Scenario แยกตาม Release
                CASE WHEN bpd.Scenario = 'ANL-RELEASE-1' THEN ISNULL(bpd.Release_Amount, 0) ELSE 0 END AS Scenario1,
                CASE WHEN bpd.Scenario = 'ANL-RELEASE-2' THEN ISNULL(bpd.Release_Amount, 0) ELSE 0 END AS Scenario2,
                CASE WHEN bpd.Scenario = 'ANL-RELEASE-3' THEN ISNULL(bpd.Release_Amount, 0) ELSE 0 END AS Scenario3,
                CASE WHEN bpd.Scenario = 'ANL-RELEASE-4' THEN ISNULL(bpd.Release_Amount, 0) ELSE 0 END AS Scenario4,
                ...
                -- Name_a1 - a4 แสดงตาม Level
                CASE
                    WHEN m.TotalLevels = 5 THEN ISNULL(m.GreatGrandparent, bap.KKU_Item_Name)
                    WHEN m.TotalLevels = 4 THEN ISNULL(m.Grandparent, bap.KKU_Item_Name)
                    WHEN m.TotalLevels = 3 THEN ISNULL(m.Parent, bap.KKU_Item_Name)
                END AS Name_a1,

                CASE 
                    WHEN (m.TotalLevels = 5 AND ISNULL(m.GreatGrandparent, bap.KKU_Item_Name) = bap.KKU_Item_Name) 
                    OR (m.TotalLevels = 4 AND ISNULL(m.Grandparent, bap.KKU_Item_Name) = bap.KKU_Item_Name)
                    OR (m.TotalLevels = 3 AND ISNULL(m.Parent, bap.KKU_Item_Name) = bap.KKU_Item_Name)
                    THEN NULL
                    WHEN m.TotalLevels = 5 THEN ISNULL(m.Grandparent, bap.KKU_Item_Name)
                    WHEN m.TotalLevels = 4 THEN ISNULL(m.Parent, bap.KKU_Item_Name)
                    WHEN m.TotalLevels = 3 THEN ISNULL(m.[Current], bap.KKU_Item_Name)
                END AS Name_a2,

                -- Name_a3
                ISNULL(
                    CASE 
                        WHEN (m.TotalLevels = 5 AND ISNULL(m.Grandparent, bap.KKU_Item_Name) = bap.KKU_Item_Name)
                        OR (m.TotalLevels = 4 AND ISNULL(m.Parent, bap.KKU_Item_Name) = bap.KKU_Item_Name)
                        OR (m.TotalLevels = 3 AND ISNULL(m.[Current], bap.KKU_Item_Name) = bap.KKU_Item_Name)
                        THEN bap.KKU_Item_Name
                        WHEN m.TotalLevels = 5 THEN ISNULL(m.Parent, bap.KKU_Item_Name)
                        WHEN m.TotalLevels = 4 THEN ISNULL(m.[Current], bap.KKU_Item_Name)
                    END,
                    bap.KKU_Item_Name
                ) AS Name_a3,
                ...
                CASE
                    ...
                    ELSE ISNULL(
                        CASE 
                            WHEN (m.TotalLevels = 5 AND ISNULL(m.Parent, bap.KKU_Item_Name) = bap.KKU_Item_Name)
                            OR (m.TotalLevels = 4 AND ISNULL(m.[Current], bap.KKU_Item_Name) = bap.KKU_Item_Name)
                            THEN NULL
                            WHEN m.TotalLevels = 5 THEN ISNULL(m.[Current], bap.KKU_Item_Name)
                        END,
                        bap.KKU_Item_Name
                    )
                END AS Name_a4
            ...
            FROM 
                ...
                LEFT JOIN budget_planning_disbursement_budget_plan_anl_release bpd 
                    ON bap.Service = bpd.Service AND bap.Faculty = bpd.Faculty 
                    AND bap.Project = bpd.Project AND bap.[Plan] = bpd.[Plan]
                    AND bap.Sub_Plan = bpd.Sub_Plan AND bap.Account = bpd.Account";
                            ...
                            <div class="card-body">
                                ...
                                <script>
                                    function validateForm(event) {
                                        ...
                                        var baseUrl = "http://202.28.118.192:8081/template-vertical-nav/report-budget-allocation.php";
                                        var params = [];
